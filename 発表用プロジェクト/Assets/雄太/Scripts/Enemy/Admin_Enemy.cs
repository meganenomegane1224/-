...
public class Admin_Enemy : MonoBehaviour
{
    ...
    {
        if(encounterBool == true && distance<= 5)
        {
            SetState(EnemyState.Standby);
        }
        else if(encounterBool == true && distance > 5)
        {
            SetState(EnemyState.Move);
        }
        else
    ...
    private void SetDestination(string text)
    {
        ...
        if(text == "Standby")
        {
            // LookPosition = new Vector3(searchChara.GetCharacterTransform().position.x , transform.position.y , searchChara.GetCharacterTransform().position.z);
